...
router.post('/', async (req, res) => {
    try {
        // Check if the incoming request body is an array (indicating multiple products)
        if (Array.isArray(req.body)) {
            // Use insertMany to save multiple products at once
            const newProducts = await Product.insertMany(req.body);
            res.status(201).json(newProducts);
        } else {
            // If it's a single product, handle it individually
            const product = new Product({
                ...
                quantity: req.body.quantity  // Include quantity if needed
            });
